...
function parseLyric(lrc) {
 ...
 console.log(parseLyric(musics[0].lrc));
 let lyrics = parseLyric(musics[0].lrc);
 //渲染到页面
...
//      div.innerHTML = `<span>${lyrics[i].content}</span>`;
//      pageLyrics.appendChild(div);
//  }
...
collectBtn.addEventListener('click', function () {
    if (toCollect) {
        collectBtn.src = 'images/MaterialSymbolsLightFavoriteOutline(0).svg';
    } else {
        collectBtn.src = 'images/MaterialSymbolsLightFavorite.svg';
    }
    toCollect = !toCollect
})
collectBtn.addEventListener('mouseover', function () {
    if (toCollect == false) {
        collectBtn.src = 'images/MaterialSymbolsLightFavoriteOutline.svg';
    }
})
...
playBtn.addEventListener('click', function () {
    if (toPlay) {
        playBtn.src = 'images/MaterialSymbolsPauseCircleRounded.svg';
        myAudio.play();
    } else {
        playBtn.src = 'images/MaterialSymbolsPlayCircleRounded.svg';
        myAudio.pause();
    }
    toPlay = !toPlay;
});
...
myAudio.addEventListener('timeupdate', function () {
    // 获取歌曲时长
    time2.textContent = musics[0].songDuration;
    // 当前播放进度
    ...
    let movePx = liHeight * currentIndex + liHeight/2 - pageLyricsHeight/2; //需要移动的
    ...
    // 实现高亮
    let currentLi = ul.children[currentIndex - 7];
    if(currentLi){
    ...
    //             pageLyrics.style.transform = `translateY(-${i * liHeight}px)`
    //         }
    //     }
    // }, 1000);
...
switchBtn.addEventListener('click', function () {
    if (toSwitch) {
        switchBtn.src = 'images/LineMdSwitchOff.svg';
        document.querySelector('.lyrics').style.color = '#a39998';
    } else {
        switchBtn.src = 'images/LineMdSwitchOffToSwitchTransition.svg';

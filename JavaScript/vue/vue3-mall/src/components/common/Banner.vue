<template>
  ...
  <div class="banner">
    <div class="wrapper">
      ...
      <ul class="pic">
        <li>
          <!-- 在组件的html代码中，@符号可以直接定位到src文件夹 -->
          <!-- <a href="#"><img src="@/assets/images/banner1.png" alt="" /></a> -->
          <!-- 动态地址用require -->
          <a href="#"><img :src="`../../../public/images/${imgs[currentIndex]}`" alt="" /></a>
        </li>
      ...
      <!-- 指示器 -->
      <ol>
        <li :class="{ current: currentIndex == index }" v-for="(item, index) in imgs" :key="index"
...
const imgs = ref(['banner1.png', 'clothes1.png', 'clothes2.png', 'clothes3.png', 'clothes4.png']);
const currentIndex = ref(0);

onMounted(() => {
  setInterval(() => {
    currentIndex.value++;
    if (currentIndex.value === imgs.value.length) {
